...
class Summary(db.Model):
    ...
    def get_text(cls, id):
        if not id:
            return None
        #summary = cls.query.get(id)
        summ = cls.query.filter_by(summary_group_id=2, doc_id=id).first()
        return "<strong>" + summ.text + "</strong>"
...
class AnnotationResult(db.Model):
    ...
    squality = db.Column(db.INTEGER, nullable=False)
    email = db.Column(db.String(255), nullable=False)
    ...
    def update_result(cls, **kwargs):
        ...
        result.validity = kwargs['validity']
        result.squality = kwargs['squality']
        result.email = kwargs['email']
        result.mturk_code = kwargs['mturk_code']
        result.is_filled = True
        db.session.commit()
