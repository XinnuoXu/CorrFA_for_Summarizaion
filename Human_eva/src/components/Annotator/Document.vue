...
//   console.log(this.$refs.docsummary)
//   console.log(this.$refs.document)
//   // this.$refs.docsummary.appendChild()
...
//   const vm1 = new HelloCtor({
//     propsData: {
//       text: 'HI :)'
//     }
//   });
//   vm1.$mount();
//   this.$refs.document.appendChild(vm1.$el)

//   const vm2 = new HelloCtor({
//     propsData: {
//       text: 'HIadafag'
//     }
//   }).$mount("#docsummary2");
...
function getFile() {
  axios.get(`project/annotation/highlight/${this.project_id}/single_doc`)
    .then((response) => {
      ...
      this.docsummary = JSON.parse(response.data.summary).text

      // showDocSummary.call(this);
      console.log(`Loaded doc doc_id: ${response.data.doc_id}`)
      console.log(`Loaded doc doc_id PARSED: ${JSON.parse(response.data.doc_id)}`)
      console.log(response.data)

      //axios.get(`summary/${response.data.doc_id}`)
      //  .then((response) => {
      //    console.log("Summary retrieved.")
      //    console.log(response)
      //    this.docsummary = response.data
      //  });
    })
    .catch(() => {
...
export default {
  ...
  data() {
    return {
      ...
      docsummary: 'asdadsfghgfdfghgf asdgh',
    };
  },
  computed: {
    timenow() {
  ...
  methods: {
      ...
      if (groupIndex !== -1) {
        this.$dialog.confirm({
          ...
          onConfirm: () => {
            ...
            this.$emit('highlight', {
              tokens: sumOfTokens,
              chunks: Object.keys(this.groups).length,
    ...
    highlightSelection() {
      ...
      if (sumOfTokens <= this.maxTokens && Object.keys(this.groups).length <= this.maxHL) {
        ...
        if (sumOfTokens > this.maxTokens){
          ...
          this.$toast.open({
            message: `Your highlights have passed the ${this.maxHL} chunks limit. Remove previous highlights if you this this one support the main point more`,
            type: 'is-danger',
          });}
